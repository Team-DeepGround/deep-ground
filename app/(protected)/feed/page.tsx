  ...
  const loadFeeds = async (pageNum: number = 0, append: boolean = false) => {
    ...
    try {
      ...
      if (res.result?.feeds) {
        ...
        const processedFeeds = res.result.feeds.map((feed) => {
          ...
          if (feed.isShared && feed.sharedFeed) {
            return {
              ...feed,
              sharedBy: {
                memberId: feed.publicId,
                memberName: feed.memberName,
                profileId: feed.profilePublicId,
              },
            };
          }
          return feed;
        ...
        if (append) {
          setFeeds((prev) => {
            const feedMap = new Map(prev.map((f) => [f.feedId, f]));
            processedFeeds.forEach((f) => feedMap.set(f.feedId, f));
